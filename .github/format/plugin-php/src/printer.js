...
function printOffsetLookup(path, options, print) {
    const node = path.getValue()
    const shouldInline = (node.offset && node.offset.kind === 'number') || getAncestorNode(path, 'encapsed')
...
function printMemberChain(path, options, print) {
    ...
    function shouldInsertEmptyLineAfter(node) {
        ...
        const nextCharIndex = getNextNonSpaceNonCommentCharacterIndex(originalText, node, options)
    ...
    function traverse(path) {
        ...
        if (node.kind === 'call' && (isLookupNode(node.what) || node.what.kind === 'call')) {
            printedNodes.unshift({
                node,
                printed: [comments.printAllComments(path, () => printArgumentsList(path, options, print), options), shouldInsertEmptyLineAfter(node) ? hardline : '']
            })
            ...
            printedNodes.unshift({
                ...
                printed: comments.printAllComments(path, () => printedMemberish, options)
            })
            path.call((what) => traverse(what), 'what')
        } else {
            printedNodes.unshift({
    ...
    for (let i = 0; i < printedNodes.length; ++i) {
        if (printedNodes[i].node.kind === 'call' && printedNodes[i - 1] && ['propertylookup', 'nullsafepropertylookup', 'staticlookup'].includes(printedNodes[i - 1].node.kind) && printedNodes[i - 1].needsParens) {
    ...
    function shouldNotWrap(groups) {
        const hasComputed = groups[1].length && groups[1][0].node.kind === 'offsetlookup'
    ...
    const isExpressionStatement = path.getParentNode().kind === 'expressionstatement'
    ...
    function printGroup(printedGroup) {
        ...
        for (let i = 0; i < printedGroup.length; i++) {
            ...
            if (printedGroup[i + 1] && printedGroup[i + 1].needsParens) {
                result.push('(', printedGroup[i].printed, printedGroup[i + 1].printed, ')')
                i++
            } else {
    ...
    const hasComment = flatGroups.slice(1, -1).some((node) => comments.hasLeadingComment(node.node)) || flatGroups.slice(0, -1).some((node) => comments.hasTrailingComment(node.node)) || (groups[cutoff] && comments.hasLeadingComment(groups[cutoff][0].node))
    ...
    // empty line after
    const lastNodeBeforeIndent = getLast(shouldMerge ? groups.slice(1, 2)[0] : groups[0]).node
    const shouldHaveEmptyLineBeforeIndent = lastNodeBeforeIndent.kind !== 'call' && shouldInsertEmptyLineAfter(lastNodeBeforeIndent)
    ...
    const callExpressionCount = printedNodes.filter((tuple) => tuple.node.kind === 'call').length
...
function printArgumentsList(path, options, print, argumentsKey = 'arguments') {
    ...
    const maybeTrailingComma = (shouldPrintComma(options, '7.3') && ['call', 'new', 'unset', 'isset'].includes(node.kind)) || (shouldPrintComma(options, '8.0') && ['function', 'closure', 'method', 'arrowfunc', 'attribute'].includes(node.kind)) ? indent([lastArg && shouldPrintHardlineBeforeTrailingComma(lastArg) ? hardline : '', ',']) : ''
    ...
    if (shouldGroupFirst || shouldGroupLast) {
        const shouldBreak = (shouldGroupFirst ? printedArguments.slice(1).some(willBreak) : printedArguments.slice(0, -1).some(willBreak)) || anyArgEmptyLine
        ...
        path.each((argPath) => {
            if (shouldGroupFirst && i === 0) {
                printedExpanded = [argPath.call((p) => print(p, { expandFirstArg: true })), printedArguments.length > 1 ? ',' : '', hasEmptyLineFollowingFirstArg ? hardline : line, hasEmptyLineFollowingFirstArg ? hardline : '', printedArguments.slice(1)]
            }

            if (shouldGroupLast && i === args.length - 1) {
                printedExpanded = [...printedArguments.slice(0, -1), argPath.call((p) => print(p, { expandLastArg: true }))]
            }
        ...
        return [
            ...
            conditionalGroup(
                [
                    !somePrintedArgumentsWillBreak ? simpleConcat : ifBreak(allArgsBrokenOut(), simpleConcat),
                    shouldGroupFirst
                        ? ['(', group(printedExpanded[0], { shouldBreak: true }), ...printedExpanded.slice(1), ')']
                        : [
                              '(',
                    ...
                    group(['(', indent([line, ...printedArguments]), ifBreak(maybeTrailingComma), line, ')'], { shouldBreak: true })
                ],
                { shouldBreak }
            )
        ]
    ...
    return group(['(', indent([softline, ...printedArguments]), ifBreak(maybeTrailingComma), softline, ')'], {
...
function printBinaryExpression(path, print, options, isNested, isInsideParenthesis) {
    ...
    if (node.kind === 'bin') {
        ...
        if (shouldFlatten(node.type, node.left.type)) {
            ...
            parts = parts.concat(path.call((left) => printBinaryExpression(left, print, options, /* isNested */ true, isInsideParenthesis), 'left'))
        } else {
        ...
        const right = shouldInline ? [node.type, ' ', path.call(print, 'right')] : [node.type, line, path.call(print, 'right')]

        // If there's only a single binary expression, we want to create a group
        // in order to avoid having a small right part like -1 be on its own line.
        const parent = path.getParentNode()
        const shouldGroup = !(isInsideParenthesis && ['||', '&&'].includes(node.type)) && getNodeKindIncludingLogical(parent) !== getNodeKindIncludingLogical(node) && getNodeKindIncludingLogical(node.left) !== getNodeKindIncludingLogical(node) && getNodeKindIncludingLogical(node.right) !== getNodeKindIncludingLogical(node)

        const shouldNotHaveWhitespace = isDocNode(node.left) || (node.left.kind === 'bin' && isDocNode(node.left.right))

        parts.push(shouldNotHaveWhitespace ? '' : ' ', shouldGroup ? group(right) : right)
...
function wrapPartsIntoGroups(parts, indexes) {
    ...
    return indexes.reduce((accumulator, index) => {
        ...
        const printedPartsForGrouping = [before || '', ...parts.slice(start, end), after || '']
        const newArray = accumulator.concat(parts.slice(lastEnd, start), alignment ? dedentToRoot(group(align(new Array(alignment).join(' '), printedPartsForGrouping))) : group(printedPartsForGrouping), end === parts.length - 1 ? parts.slice(end) : '')
...
function printLines(path, options, print, childrenAttribute = 'children') {
    ...
    path.map((childPath, index) => {
        ...
        const canPrintBlankLine = !isLastStatement(childPath) && !isInlineNode && (nextNode && nextNode.kind === 'case' ? !isFirstChildrenInlineNode(path) : nextNode && nextNode.kind !== 'inline')

        let printed = [printedPath, canPrintBlankLine ? hardline : '', canPrintBlankLine && isNextLineEmpty(options.originalText, childNode, options) ? hardline : '']

        const isFirstNode = index === 0
        const isLastNode = children.length - 1 === index
        const isBlockNestedNode = node.kind === 'block' && parentNode && ['function', 'closure', 'method', 'try', 'catch'].includes(parentNode.kind)
        ...
        if (isInlineNode || (!isInlineNode && isLastNode && lastInlineIndex >= 0)) {
            ...
            const shouldCreateGroup = (isInlineNode && !isFirstNode) || (!isInlineNode && isLastNode)

            if (shouldCreateGroup) {
                const start = (isInlineNode ? prevLastInlineIndex : lastInlineIndex) + 1
                const end = isLastNode && !isInlineNode ? index + 1 : index
                const prevInlineNode = children[isInlineNode ? prevLastInlineIndex : lastInlineIndex]
                const alignment = prevInlineNode ? getAlignment(prevInlineNode.raw) : ''
                const shouldBreak = end - start > 1
                const before = shouldBreak ? ((isBlockNestedNode && !prevInlineNode) || (isProgramLikeNode(node) && start === 0) ? '' : hardline) : ''
                const after = shouldBreak && childNode.kind !== 'halt' ? (isBlockNestedNode && isLastNode ? '' : hardline) : ''
        ...
        if (isInlineNode) {
            ...
            const beforeInline = childNode.leadingComments && childNode.leadingComments.length ? [isFirstNode && node.kind !== 'namespace' && !isBlockNestedNode ? '<?php' : '', node.kind === 'namespace' || !isBlockNestedNode ? hardline : '', comments.printComments(childNode.leadingComments, options), hardline, '?>'] : isProgramLikeNode(node) && isFirstNode && node.kind !== 'namespace' ? '' : [beforeCloseTagInlineNode, '?>']

            const nextV = path.getNode(index + 1)
            const skipLastComment = nextV && nextV.children && nextV.children.length

            const afterInline = childNode.comments && childNode.comments.length ? [openTag, hardline, skipLastComment ? comments.printComments(childNode.comments, options) : '', hardline] : isProgramLikeNode(node) && isLastNode ? '' : [openTag, ' ']
    ...
    if (node.kind === 'program' && !node.extra.parseAsEval) {
        ...
        if (hasStartTag) {
            ...
            const afterOpenTag = [between && between[2] && between[2].includes('\n') ? [hardline, between[2].split('\n').length > 2 ? hardline : ''] : ' ', node.comments ? comments.printComments(node.comments, options) : '']

            const shortEcho = firstNode && firstNode.kind === 'echo' && firstNode.shortForm
        ...
        if (hasEndTag) {
            ...
            const beforeCloseTag = lastNode ? [hasNewlineInRange(options.originalText.trimEnd(), options.locEnd(lastNode), options.locEnd(node)) ? (!(lastNode.kind === 'inline' && lastNode.comments && lastNode.comments.length) ? hardline : '') : ' ', isNextLineEmpty(options.originalText, lastNode, options) ? hardline : ''] : node.comments ? hardline : ''
...
function printClassPart(path, options, print, part = 'extends', beforePart = ' ', afterPart = ' ') {
    const node = path.getValue()
    const printedBeforePart = hasDanglingComments(node[part]) ? [hardline, path.call((partPath) => comments.printDanglingComments(partPath, options, true), part), hardline] : beforePart
...
function printClass(path, options, print) {
    ...
    if (node.extends && node.implements) {
        declaration.push(
            conditionalGroup(
                [
                    [printClassPart(path, options, print, 'extends'), printClassPart(path, options, print, 'implements')],
                    [printClassPart(path, options, print, 'extends'), printClassPart(path, options, print, 'implements', ' ', hardline)],
                    [printClassPart(path, options, print, 'extends', hardline, ' '), printClassPart(path, options, print, 'implements', hardline, node.implements.length > 1 ? hardline : ' ')]
                ],
                {
    ...
    const printedDeclaration = group([group(declaration), shouldPrintHardlineForOpenBrace(options) ? (isAnonymousClass ? line : hardline) : ' '])

    const hasEmptyClassBody = node.body && node.body.length === 0 && !hasDanglingComments(node)
    const printedBody = ['{', indent([hasEmptyClassBody ? '' : hardline, printStatements(path, options, print, 'body')]), comments.printDanglingComments(path, options, true), isAnonymousClass && hasEmptyClassBody ? softline : hardline, '}']
...
function printFunction(path, options, print) {
    ...
    if (node.type) {
        declaration.push([': ', hasDanglingComments(node.type) ? [path.call((typePath) => comments.printDanglingComments(typePath, options, true), 'type'), ' '] : '', node.nullable ? '?' : '', path.call(print, 'type')])
    }
    ...
    const printedBody = ['{', indent([hasEmptyBody(path) ? '' : hardline, path.call(print, 'body')]), isClosure && hasEmptyBody(path) ? '' : hardline, '}']
    ...
    if (node.arguments.length === 0) {
        return [...declAttrs, printedDeclaration, shouldPrintHardlineForOpenBrace(options) ? hardline : ' ', printedBody]
    }
    ...
    return [
        ...declAttrs,
        conditionalGroup([
            [printedDeclaration, shouldPrintHardlineForOpenBrace(options) ? hardline : ' ', printedBody],
            [printedDeclaration, ' ', printedBody]
        ])
    ]
}

function printBodyControlStructure(path, options, print, bodyProperty = 'body') {
    const node = path.getValue()
...
function printAssignment(leftNode, printedLeft, operator, rightNode, printedRight, hasRef, options) {
    ...
    const printed = printAssignmentRight(leftNode, rightNode, printedRight, hasRef, options)
...
function needsHardlineAfterDanglingComment(node) {
    ...
    const lastDanglingComment = getLast(node.comments.filter((comment) => !comment.leading && !comment.trailing))
...
function isStringOnItsOwnLine(node, text, options) {
    return (node.kind === 'string' || (node.kind === 'encapsed' && (node.type === 'string' || node.type === 'shell'))) && stringHasNewLines(node) && !hasNewline(text, options.locStart(node), { backwards: true })
}

function printComposedTypes(path, print, glue) {
    return group(path.map((uPath, i) => (i === 0 ? [path.call(print)] : [glue, path.call(print)]), 'types'))
}
...
function printNode(path, options, print) {
    ...
    switch (node.kind) {
        case 'program': {
            return group([printLines(path, options, print), comments.printDanglingComments(path, options, /* sameIndent */ true, (c) => !c.printed)])
        }
        ...
        case 'usegroup':
            return group([
                ...
                node.type ? [node.type, ' '] : '',
                indent([
                    node.name ? [maybeStripLeadingSlashFromUse(node.name), '\\{', softline] : '',
                    join(
                        [',', line],
                        path.map((item) => print(item), 'items')
                    )
                ]),
                node.name ? [ifBreak(shouldPrintComma(options, '7.2') ? ',' : ''), softline, '}'] : ''
            ])
        case 'useitem':
        ...
        case 'traitalias':
            return [node.trait ? [path.call(print, 'trait'), '::'] : '', node.method ? path.call(print, 'method') : '', ' as ', join(' ', [...(node.visibility ? [node.visibility] : []), ...(node.as ? [path.call(print, 'as')] : [])])]
        case 'traituse':
        ...
        case 'arrowfunc':
            return [node.parenthesizedExpression ? '(' : '', ...printAttrs(path, options, print, { inline: true }), node.isStatic ? 'static ' : '', 'fn', printArgumentsList(path, options, print), node.type ? [': ', node.nullable ? '?' : '', path.call(print, 'type')] : '', ' => ', path.call(print, 'body'), node.parenthesizedExpression ? ')' : '']
        case 'parameter': {
            let promoted = ''
            if (node.flags === 1) {
                promoted = 'public '
            ...
            const name = [...printAttrs(path, options, print, { inline: true }), promoted, node.readonly ? 'readonly ' : '', node.nullable ? '?' : '', node.type ? [path.call(print, 'type'), ' '] : '', node.byref ? '&' : '', node.variadic ? '...' : '', '$', path.call(print, 'name')]
            ...
            if (node.value) {
                return group([
                    ...
                    ' =',
                    printAssignmentRight(node.name, node.value, path.call(print, 'value'), false, options)
                ])
            }
        ...
        case 'propertystatement': {
            const attrs = []
            path.map((propPath) => attrs.push(...printAttrs(propPath, options, print)), 'properties')
            ...
            return group([...attrs, hasVisibility ? [node.visibility === null ? 'var' : node.visibility, ''] : '', node.isStatic ? [hasVisibility ? ' ' : '', 'static'] : '', firstProperty ? [' ', firstProperty] : '', indent(printed.slice(1).map((p) => [',', hasValue ? hardline : line, p]))])
        }
        case 'if': {
            const parts = []
            ...
            if (node.alternate) {
                ...
                const commentOnOwnLine = (hasTrailingComment(node.body) && node.body.comments.some((comment) => comment.trailing && !comments.isBlockComment(comment))) || needsHardlineAfterDanglingComment(node)
        ...
        case 'for': {
            ...
            return [printedComments, group(['for (', group([indent([softline, group(join([',', line], path.map(print, 'init'))), ';', line, group(join([',', line], path.map(print, 'test'))), ';', line, group(join([',', line], path.map(print, 'increment')))]), softline]), ')', body])]
        }
        case 'foreach': {
            const body = printBodyControlStructure(path, options, print, 'body')
            ...
            return [printedComments, group(['foreach (', group([indent([softline, path.call(print, 'source'), line, 'as ', group(node.key ? indent(join([' =>', line], [path.call(print, 'key'), path.call(print, 'value')])) : path.call(print, 'value'))]), softline]), ')', body])]
        }
        case 'try': {
            const parts = []

            parts.push('try', printBodyControlStructure(path, options, print, 'body'))
            ...
            if (node.always) {
                parts.push(' finally', printBodyControlStructure(path, options, print, 'always'))
            }
        ...
        case 'new': {
            const isAnonymousClassNode = node.what && node.what.kind === 'class' && node.what.isAnonymous

            // Multiline strings as single arguments
            if (!isAnonymousClassNode && node.arguments.length === 1 && isStringOnItsOwnLine(node.arguments[0], options.originalText, options)) {
                return ['new ', ...path.call(printAttrs, 'what'), path.call(print, 'what'), '(', join(', ', path.map(print, 'arguments')), ')']
            }
            ...
            if (isAnonymousClassNode) {
                parts.push(node.what.leadingComments && node.what.leadingComments[0].kind === 'commentblock' ? [comments.printComments(node.what.leadingComments, options), ' '] : '', ...path.call((pa) => printAttrs(pa, options, print, { inline: true }), 'what'), 'class', node.arguments.length > 0 ? [' ', printArgumentsList(path, options, print)] : '', group(path.call(print, 'what')))
            } else {
                const isExpression = ['call', 'offsetlookup'].includes(node.what.kind)
                const printed = [isExpression ? '(' : '', path.call(print, 'what'), isExpression ? ')' : '', printArgumentsList(path, options, print)]
        ...
        case 'offsetlookup': {
            ...
            const hasEncapsedAncestor = getAncestorNode(path, 'encapsed')
            const shouldInline = hasEncapsedAncestor || (firstNonMemberParent && (firstNonMemberParent.kind === 'new' || (firstNonMemberParent.kind === 'assign' && firstNonMemberParent.left.kind !== 'variable'))) || node.kind === 'offsetlookup' || ((isReferenceLikeNode(node.what) || node.what.kind === 'variable') && ['identifier', 'variable', 'encapsedpart'].includes(node.offset.kind) && parent && !isLookupNode(parent))
        ...
        case 'halt':
            return [hasDanglingComments(node) ? [comments.printDanglingComments(path, options, /* sameIndent */ true), hardline] : '', '__halt_compiler();', node.after]
        case 'eval':
        ...
        case 'echo': {
            ...
            return group([node.shortForm ? '' : 'echo ', firstVariable ? firstVariable : '', indent(printedArguments.slice(1).map((p) => [',', line, p]))])
        }
        case 'print': {
        ...
        case 'variable': {
            ...
            const dollar = (parent.kind === 'encapsedpart' && parent.syntax === 'simple' && parent.curly) || (parentParent && parent.kind === 'offsetlookup' && parentParent.kind === 'encapsedpart' && parentParent.syntax === 'simple' && parentParent.curly) ? '' : '$'
        ...
        case 'classconstant': {
            ...
            return group([...attrs, node.visibility ? [node.visibility, ' '] : '', 'const', firstVariable ? [' ', firstVariable] : '', indent(printed.slice(1).map((p) => [',', hardline, p]))])
        }
        case 'constant':
        ...
        case 'static': {
            ...
            return group(['static', firstVariable ? [' ', firstVariable] : '', indent(printed.slice(1).map((p) => [',', hasValue ? hardline : line, p]))])
        }
        case 'staticvariable': {
        ...
        case 'list':
        case 'array': {
            const useShortForm = (node.kind === 'array' && isMinVersion(options.phpVersion, '5.4')) || (node.kind === 'list' && (node.shortForm || isMinVersion(options.phpVersion, '7.1')))
            ...
            if (node.items.length === 0) {
                ...
                return group([open, comments.printDanglingComments(path, options), hardline, close])
            }
            ...
            const [firstProperty] = node.items.filter((node) => node.kind !== 'noop').sort((a, b) => options.locStart(a) - options.locStart(b))
            const isAssociative = !!(firstProperty && firstProperty.key)
            const shouldBreak = isAssociative && firstProperty && hasNewlineInRange(options.originalText, options.locStart(node), options.locStart(firstProperty))

            return group([open, indent([softline, printArrayItems(path, options, print)]), needsForcedTrailingComma ? ',' : '', ifBreak(!needsForcedTrailingComma && shouldPrintComma(options, '5.0') ? [lastElem && shouldPrintHardlineBeforeTrailingComma(lastElem) ? softline : '', ','] : ''), comments.printDanglingComments(path, options, true), softline, close], { shouldBreak })
        }
        case 'entry': {
        ...
        case 'yield': {
            const printedKeyAndValue = [node.key ? [path.call(print, 'key'), ' => '] : '', path.call(print, 'value')]
        ...
        case 'bin': {
            const parent = path.getParentNode()
            const parentParent = path.getParentNode(1)
            const isInsideParenthesis = node !== parent.body && (parent.kind === 'if' || parent.kind === 'while' || parent.kind === 'switch' || parent.kind === 'do')

            const parts = printBinaryExpression(path, print, options, /* isNested */ false, isInsideParenthesis)
            ...
            //     c
            //   )->call()
            if (parent.kind === 'unary' || (isLookupNode(parent) && parent.kind !== 'offsetlookup')) {
                return group([indent([softline, ...parts]), softline])
            }

            // Avoid indenting sub-expressions in some cases where the first sub-expression is already
            // indented accordingly. We should indent sub-expressions where the first case isn't indented.
            const shouldNotIndent = (node !== parent.body && parent.kind === 'for') || (parent.kind === 'retif' && parentParent && parentParent.kind !== 'return')

            const shouldIndentIfInlining = ['assign', 'property', 'constant', 'staticvariable', 'entry'].includes(parent.kind)

            const samePrecedenceSubExpression = node.left.kind === 'bin' && shouldFlatten(node.type, node.left.type)
        ...
        case 'retif': {
            ...
            const printedFalseExpr = node.falseExpr.kind === 'bin' ? indent(path.call(print, 'falseExpr')) : path.call(print, 'falseExpr')
            const part = [node.trueExpr ? line : ' ', '?', node.trueExpr ? [' ', node.trueExpr.kind === 'bin' ? indent(path.call(print, 'trueExpr')) : path.call(print, 'trueExpr'), line] : '', ':', node.trueExpr ? [' ', printedFalseExpr] : [shouldInlineRetifFalseExpression(node.falseExpr) ? ' ' : line, printedFalseExpr]]

            parts.push(part)

            // We want a whole chain of retif to all break if any of them break.
            const maybeGroup = (doc) => (parent === firstNonRetifParent ? group(doc) : doc)
            ...
            // )->call()
            const parentParent = path.getParentNode(1)
            const pureParent = parent.kind === 'cast' && parentParent ? parentParent : parent
            const breakLookupNodes = ['propertylookup', 'nullsafepropertylookup', 'staticlookup']
            const breakClosingParens = breakLookupNodes.includes(pureParent.kind)

            const printedTest = path.call(print, 'test')

            if (!node.trueExpr) {
                const printed = [printedTest, pureParent.kind === 'bin' || ['print', 'echo', 'return', 'include'].includes(firstNonRetifParent.kind) ? indent(parts) : parts]
            ...
            return maybeGroup([node.test.kind === 'retif' ? indent(printedTest) : printedTest, indent(parts), breakClosingParens ? softline : ''])
        }
        case 'boolean':
        ...
        case 'string': {
            ...
            if (parent.kind === 'encapsedpart') {
                ...
                return join(
                    linebreak,
                    node.raw.split('\n').map((s, i) => (i > 0 || node.loc.start.column === 0 ? s.substring(closingTagIndentation) : s))
                )
            }
            ...
            return [node.raw[0] === 'b' ? 'b' : '', quote, stringValue, quote]
        }
        ...
        case 'encapsedpart': {
            const open = (node.syntax === 'simple' && node.curly) || node.syntax === 'complex' ? [node.curly ? '$' : '', '{'] : ''
            const close = (node.syntax === 'simple' && node.curly) || node.syntax === 'complex' ? '}' : ''
        ...
        case 'nowdoc': {
            const flexible = isMinVersion(options.phpVersion, '7.3')
            const linebreak = flexible ? hardline : literalline
            return ["<<<'", node.label, "'", linebreak, join(linebreak, node.value.split('\n')), linebreak, node.label, docShouldHaveTrailingNewline(path) ? hardline : '']
        }
        case 'name':
        ...
        case 'match': {
            const arms = path.map((armPath, armIdx) => {
                const conds = armPath.getValue().conds === null ? 'default' : armPath.map((condPath, condIdx) => [',', line, print(condPath)].slice(condIdx === 0 ? 2 : 0), 'conds')
                const body = armPath.call(print, 'body')
                return [',', hardline, group([group([conds, indent(line)]), '=> ', body])].slice(armIdx > 0 ? 0 : 1)
            }, 'arms')
            return group(['match (', group([softline, indent(path.call(print, 'cond')), softline]), ') {', group(indent([...arms, options.trailingCommaPHP ? ifBreak(',') : ''])), ' ', softline, '}'])
        }
